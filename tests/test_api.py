...
class APITester:
    ...
    def test_model_info_endpoint(self) -> bool:
        ...
        try:
            response = self.session.get(f"{self.base_url}/v1/model-info", timeout=self.timeout)
    ...
    def prepare_minimal_prediction_data(
    ...
    def test_main_prediction(self, records: List[Dict[str, Any]], batch_size: int = 5) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/v1/predict", json=payload, timeout=self.timeout
            )
    ...
    def test_minimal_prediction(self, records: List[Dict[str, Any]], batch_size: int = 5) -> bool:
        ...
        try:
            ...
            print(f"   Sending {len(test_records)} records...")
            response = self.session.post(
                f"{self.base_url}/v1/predict-minimal", json=payload, timeout=self.timeout
            )
    ...
    def test_error_handling(self) -> bool:
        ...
        test_cases = [
            {"name": "Empty records", "payload": {"records": []}, "expected_status": 422},
            {
            ...
            },
            {
                ...
                "payload": {
                    "records": [
                        {"bedrooms": 3, "bathrooms": 2.0, "sqft_living": 1500, "zipcode": "invalid"}
                    ]
                },
                "expected_status": 422,
            },
            {
                ...
                "payload": {
                    "records": [
                        {"bedrooms": -1, "bathrooms": 2.0, "sqft_living": 1500, "zipcode": "98001"}
                    ]
                },
                "expected_status": 422,
            },
        ...
        for test_case in test_cases:
            try:
                response = self.session.post(
                    f"{self.base_url}/v1/predict", json=test_case["payload"], timeout=self.timeout
                )
    ...
    def run_comprehensive_test(self, csv_path: str) -> bool:
        ...
        tests = [
            ...
            (
                "Main Prediction",
                lambda: self.test_main_prediction(self.prepare_main_prediction_data(raw_records)),
            ),
            (
                "Minimal Prediction",
                lambda: self.test_minimal_prediction(
