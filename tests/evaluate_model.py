...
class ModelEvaluator:
    ...
    def _prepare_data(self) -> None:
        ...
        sales_data = pd.read_csv(
            self.data_dir / "kc_house_data.csv",
        ...
        demographics = pd.read_csv(
            self.data_dir / "zipcode_demographics.csv",
            dtype={"zipcode": str}
        )
    ...
    def cross_validation_analysis(self, cv_folds: int = 5) -> Dict[str, Any]:
        ...
        for metric in scoring:
            ...
            cv_stats[metric] = {
                ...
                'stability': test_scores.std() / test_scores.mean() if test_scores.mean() != 0 else float('inf')
            }
    ...
    def feature_importance_analysis(self) -> pd.DataFrame:
        ...
        for i, (_, row) in enumerate(importance_df.head(10).iterrows()):
            print(f"      {i+1:2d}. {row['feature']:<25} {row['importance_mean']:8.0f} ± {row['importance_std']:6.0f}")
    ...
    def save_report(self, report: Dict[str, Any], output_path: str = "model_evaluation_report.json") -> None:
